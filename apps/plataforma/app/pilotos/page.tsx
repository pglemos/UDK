...
import { DriverPoster, EditorialEmpty, EditorialHeading } from "../../components/race/editorial-primitives";
      ...
      <main id="conteudo" className="udk-page tg-internal-page">
          ...
          <div className="race-container">
            ...
            {drivers.items.length ? (
              <section className="tg-driver-directory-grid" aria-label="Pilotos">
                {drivers.items.map((driver, index) => <DriverPoster driver={driver} index={index} key={driver.slug} />)}
              </section>
            ) : (
              <EditorialEmpty
                index="04"
        ...
        <section className="tg-inline-cta is-dark">
          <div className="race-container">
            ...
            <div><span>O próximo perfil</span><h2>Seu nome também pode ocupar este grid.</h2></div>
            <Link href="/inscricao" className="race-button race-button-primary">Entrar no campeonato <ArrowRight aria-hidden="true" /></Link>
          </div>
        </section>
      </main>
    </RaceShell>
